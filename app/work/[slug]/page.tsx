...
function LiveWebsitePreview({ url, title }: { url: string, title: string }) {
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex gap-2">
          <button
            ...
            className="text-xs px-3 py-1 bg-neutral-100 dark:bg-neutral-800 rounded-md hover:bg-neutral-200 dark:hover:bg-neutral-700 transition-colors"
          >
          ...
          <a
            ...
            className="text-xs px-3 py-1 bg-neutral-900 text-white dark:bg-neutral-100 dark:text-black rounded-md hover:bg-neutral-800 dark:hover:bg-neutral-200 transition-colors"
          >
            open in new tab ↗
          </a>
        </div>
      ...
      <AnimatePresence>
        {showIframe && (
          <motion.div
            ...
            className="relative overflow-hidden bg-neutral-100 dark:bg-neutral-800 rounded-lg border border-neutral-200 dark:border-neutral-700"
          >
            {!isLoaded && (
...
function CopyProjectLink({ url, title }: { url: string, title: string }) {
  ...
  return (
    <motion.button
      onClick={copyLink}
      className="flex items-center gap-2 text-xs px-3 py-2 bg-neutral-100 dark:bg-neutral-800 hover:bg-neutral-200 dark:hover:bg-neutral-700 rounded-md transition-colors"
...
function ProjectStats({ project }: { project: Project }) {
  const stats = [
    { label: "technologies", value: project.technologies?.length || 0 },
    { label: "year", value: project.year || "2024" },
    { label: "type", value: project.technologies?.includes("E-commerce") ? "e-commerce" : project.technologies?.includes("React") ? "web app" : "website" },
    { label: "status", value: "live" }
  ];

  return (
    <motion.div 
      className="grid grid-cols-2 md:grid-cols-4 gap-4 p-6 bg-neutral-50 dark:bg-neutral-800/50 rounded-lg border border-neutral-200 dark:border-neutral-700 mb-16"
...
// Clean image carousel with smooth animations and lightbox
        ...
        <AnimatePresence initial={false} custom={direction} mode="wait">
          ...
          >
            <div className="relative w-full h-full max-w-4xl">
              ...
              />
              {/* Zoom indicator */}
              <div className="absolute bottom-4 right-4 bg-black/50 text-white text-xs px-2 py-1 rounded opacity-0 hover:opacity-100 transition-opacity">
                click to zoom
              </div>
            </div>
          </motion.div>
        ...
        <div className="pt-24 px-6 md:px-12 max-w-6xl mx-auto">
          ...
          >
            ...
            >
              ...
              <CopyProjectLink url={project.link} title={project.title} />
